...
Generated by PACE Orchestrator — Day {day}"""
    ...
    def push_story(self, day: int, day_dir: Path) -> str:
        if not self._available:
            print("[Bitbucket] push_story: adapter not configured — skipping.")
            return ""
        story_file = day_dir / "story.md"
        if not story_file.exists():
            print(f"[Bitbucket] push_story: story.md not found in {day_dir}")
            return ""
        ...
        try:
            ...
            self._save_ticket_ref(day_dir, {"url": url, "id": issue_id, "platform": "bitbucket"})
            print(f"[Bitbucket] Story ticket opened: {url}")
            return url
        except Exception as e:
            print(f"[Bitbucket] push_story failed: {e}")
            return ""
    ...
    def update_story_status(self, day: int, day_dir: Path, status: str) -> None:
        ...
        issue_id = ref.get("id")
        if not issue_id:
            print(f"[Bitbucket] update_story_status: no ticket reference for Day {day}")
            return
        bb_status = "resolved" if status == "done" else "open"
        try:
            _requests.put(
                self._repo_url(f"issues/{issue_id}"),
                headers=self._headers(),
                json={"status": bb_status},
                timeout=15,
            )
            print(f"[Bitbucket] Story issue #{issue_id} status → '{bb_status}' (Day {day})")
        except Exception as e:
            print(f"[Bitbucket] update_story_status failed: {e}")
    ...
    def post_handoff_comment(self, day: int, day_dir: Path) -> None:
        ...
        if not issue_id or not handoff_file.exists():
            print(f"[Bitbucket] post_handoff_comment: missing ref or handoff.md for Day {day}")
            return
        from issue_template import handoff_comment_markdown
        handoff = yaml.safe_load(handoff_file.read_text()) or {}
        try:
            _requests.post(
                self._repo_url(f"issues/{issue_id}/comments"),
                headers=self._headers(),
                json={"content": {"raw": handoff_comment_markdown(day, handoff)}},
                timeout=15,
            )
            print(f"[Bitbucket] Handoff comment posted to issue #{issue_id} (Day {day})")
        except Exception as e:
            print(f"[Bitbucket] post_handoff_comment failed: {e}")


class BitbucketBranchingAdapter(_BitbucketBase, BranchingAdapter):
    """Manages the release/sprint branch hierarchy via the Bitbucket Cloud Refs API."""
    ...
    def get_branch_sha(self, branch: str) -> str | None:
        ...
        try:
            resp = _requests.get(
                self._repo_url(f"refs/branches/{branch}"),
                headers=self._headers(),
                timeout=15,
            )
            ...
            print(f"[Bitbucket] get_branch_sha({branch!r}): {e}")
            return None
    ...
    def create_branch(self, new_branch: str, from_branch: str) -> None:
        ...
        if not self._available:
            print(f"[Bitbucket] Adapter not configured — skipping create_branch({new_branch!r})")
            return
        sha = self.get_branch_sha(from_branch)
        if not sha:
            print(
                f"[Bitbucket] create_branch: source '{from_branch}' not found — "
                f"cannot create '{new_branch}'"
            )
            return
        try:
            resp = _requests.post(
                self._repo_url("refs/branches"),
                headers=self._headers(),
                json={"name": new_branch, "target": {"hash": sha}},
                timeout=15,
            )
            if resp.status_code == 409:
                print(f"[Bitbucket] Branch '{new_branch}' already exists — skipping")
                return
            resp.raise_for_status()
            print(f"[Bitbucket] Branch created: {new_branch} (from {from_branch} @ {sha[:8]})")
        except Exception as e:
            print(f"[Bitbucket] create_branch({new_branch!r}): {e}")
    ...
    def create_pull_request(
        ...
        Note: Bitbucket Cloud does not support labels on PRs; the labels
        argument is accepted but ignored.
        """
        if not self._available:
            print(f"[Bitbucket] Adapter not configured — skipping PR: {head} → {base}")
            return ""
        try:
            resp = _requests.post(
                self._repo_url("pullrequests"),
            ...
            if resp.status_code == 409:
                print(f"[Bitbucket] PR {head} → {base} already exists — skipping")
                return ""
            resp.raise_for_status()
            url = resp.json().get("links", {}).get("html", {}).get("href", "")
            print(f"[Bitbucket] PR opened: {url}")
            return url
        except Exception as e:
            print(f"[Bitbucket] create_pull_request({head!r} → {base!r}): {e}")
            return ""
