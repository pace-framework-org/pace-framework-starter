...
2. Close this issue
3. Go to **Settings → CI/CD → Variables** and set `PACE_PAUSED` to `false`
...
*Generated by PACE Orchestrator — Day {day}*"""
    ...
    def set_variable(self, name: str, value: str) -> bool:
        """Set a GitLab CI/CD project variable via python-gitlab.

        Updates the variable if it already exists; creates it otherwise.
        Returns True on success, False on failure (non-fatal — logs the error).
        """
        if not self._project:
            print(f"[GitLab] No project configured — cannot set variable {name}.")
            return False
        try:
            try:
                var = self._project.variables.get(name)
                var.value = value
                var.save()
            except Exception:
                self._project.variables.create({"key": name, "value": value})
            print(f"[GitLab] Variable {name} set to {value!r}.")
            return True
        except Exception as e:
            print(f"[GitLab] Failed to set variable {name}: {e}")
            return False
