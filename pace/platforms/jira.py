...
class JiraAdapter(PlatformAdapter):
    ...
    def set_variable(self, name: str, value: str) -> bool:
        """Jira has no concept of pipeline variables. Log the intent and return False
        (non-fatal). Use a GitHub/GitLab adapter for variable management."""
        print(
            f"[Jira] set_variable({name!r}) — Jira does not manage CI/CD variables. "
            f"Set {name}={value!r} manually in your CI/CD platform (GitHub/GitLab/Jenkins)."
        )
        return False
